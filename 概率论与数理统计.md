...
|平均分布U(a,b) | ${a+b}\over 2$ | $(b-a)^2 \over 12$ | $f(x)=\{_0^{1\over (b-a)}$ |
|指数分布 $E(\theta)$ | $\theta$ | $\theta$ | $f(x)={1\over \theta}e^{-x\over \theta}$ |
|正态分布 $N(\mu,\sigma ^2)$ | $\mu$ | $\sigma ^2$ | $f(x)={1\over {\sqrt{2\pi}\sigma}}e^{-{{(x-\mu)^2}\over \sigma ^2}}$ |
...
相关系数：
$$\rho_{XY}={\rm{Cov(X,Y)} \over {\sqrt{D(X)}\sqrt{D(X)}}}$$
$$\rho_{XY}=0 \quad\Rightarrow E(XY)=E(X)E(Y)$$

### 1.2 性质：
1. 相关性描述变量之间的线性相关程度，独立性描述变量之间独立的关系
   即$\rho=0 \rightarrow 不相关\rightarrow 无线性关系\nrightarrow 相互独立$
2. $\rm{Cov}(aX,bY)=ab\rm{Cov}(X,Y)$
3. $\rm{Cov}(X_1+X_2,Y)=\rm{Cov}(X_1,Y)+\rm{Cov}(X_2,Y)$
4. $\rm{Cov}(X,X)=D(X)$
5. 用$a+bX$表达$Y$时，近似最好的：
    $$b={\rm{Cov(X,Y)} \over {\sqrt{D(X)}}}$$
    $$a=E(Y)-E(X)*b$$
...
$$\eta_n =\sum^n X_k$$
$$\lim_{n\rightarrow \infty}P \left\{{{\eta_n-np}\over \sqrt{np(1-p)}}\leqslant x\right\}=\varPhi(x)$$

即

$$\lim_{n\rightarrow \infty}P \left\{{{\eta_n-E(\eta_n)}\over \sqrt{D(\eta_n)}}\leqslant x\right\}=\varPhi(x)$$
...
两种错误：
第Ⅰ类错误：弃真， $P\left\{当H_0为真时拒绝H_0\right\}\leqslant \alpha$
第Ⅱ类错误：取伪， $P\left\{当H_0为假时接收H_0\right\}\leqslant \beta$
